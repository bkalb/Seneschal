...
const server = setupServer(
  ...
  http.post("/api/saved-npc", async ({ request }) => {
    ...
    const npc: DbNpc = {
      ...
      isPinned: false,
      notes: null,
      isCombatant: body.isCombatant ?? false,
      combatAc: body.combatAc ?? null,
      combatHd: body.combatHd ?? null,
      combatMaxHp: body.combatMaxHp ?? null,
      combatAttackBonus: body.combatAttackBonus ?? null,
      combatAttackDamage: body.combatAttackDamage ?? null,
      createdAt: now,
      updatedAt: now,
    };
    npcStore.push(npc);
  ...
  http.post("/api/saved-npc/batch", async ({ request }) => {
    ...
    const created = body.npcs.map((n) => {
      const npc: DbNpc = {
        ...
        isPinned: false,
        notes: null,
        isCombatant: n.isCombatant ?? false,
